...
unsafe extern "C" fn gaogaen_game_attackairf(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn gaogaen_effect_attackairf(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
...
pub fn install() {
    Agent::new("gaogaen")
        .game_acmd("game_attackairf", gaogaen_game_attackairf)
        .effect_acmd("effect_attackairf", gaogaen_effect_attackairf)
        .install();
}
